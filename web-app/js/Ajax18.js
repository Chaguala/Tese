...
function processCronogramaEditing(component, maxSize) {
    ...
    var currentLine = parseInt(strs[1])
    var inicio = parseInt(strs[2]) + 1;
    var meioSemestre = (maxSize / 2) - 1;
    var fimSemestre = maxSize - 1;
    var sugestao = meioSemestre;

    if (inicio > meioSemestre) sugestao = fimSemestre;


    var question = 'Repetir o valor digitado de ' + inicio + ' ate ? (max ' + maxSize + ')';

    if (inicio < maxSize) {
        var repitUntil = window.prompt(question, sugestao);

        if (isNumber(repitUntil)) {
            if (repitUntil <= maxSize) {
                for (var i = inicio; i < repitUntil; i++) {
                    document.getElementById('planoSemana.qtdHoras_' + currentLine + '_' + i).value = component.value;
                }
                document.getElementById('planoSemana.qtdHoras_' + currentLine + '_' + repitUntil).focus();
            }
        }
    }
}
